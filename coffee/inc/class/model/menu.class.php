<?php
    ...
    class menu extends model {
        ...
        public function listByTree($user_id, $ref_menu_id = null){
            $sql = " SELECT DISTINCT"
                    . "    m.menu_id, m.ref_menu_id, m.menu_code, m.menu_name, m.href"
            ...
            while($row = $this->db->fetch_array($result)){
                $row["child_menu"] = $this->listByTree($user_id, $row["menu_id"]);
                $arr[] = $row;
            }
        ...
        public function isDuplicate($menu){
            $table = "tb_menu";

            # check duplicate menu_code
            $condition = "status <> 'D' AND menu_code = '{$menu["menu_code"]}'";
            if (strUtil::isNotEmpty($menu["menu_id"])){
                $condition .= " AND menu_id <> '{$menu["menu_id"]}'";
            }
            ...
            $condition = "status <> 'D' AND menu_name = '{$menu["menu_name"]}'";
            if (strUtil::isNotEmpty($menu["menu_id"])){
                $condition .= " AND menu_id <> '{$menu["menu_id"]}'";
            }
        ...
        public function getById($menu_id){
            $field = "m.menu_id, m.menu_code, m.menu_name, m.ref_menu_id, m.href, m.status"
                       . ", vs1.user_name AS created_by, m.created_date"
                       . " , vs2.user_name AS modified_by, m.modified_date";
            $table = " tb_menu m"
                        . " LEFT JOIN tb_user vs1 ON (m.created_by = vs1.user_id)"
                        . " LEFT JOIN tb_user vs2 ON (m.modified_by = vs2.user_id)";
        ...
        public function insert(&$menu){
            $table = "tb_menu";
            $field = "menu_code, menu_name, ref_menu_id, href, status, created_by, created_date, modified_by, modified_date";
            $data = "'{$menu["menu_code"]}', '{$menu["menu_name"]}', {$menu["ref_menu_id"]}, '{$menu["href"]}', '{$menu["status"]}', '{$menu["action_by"]}', '{$menu["action_date"]}', '{$menu["action_by"]}', '{$menu["action_date"]}'";
